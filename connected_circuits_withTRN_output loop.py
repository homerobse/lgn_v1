...
for nsim in range(nruns):
    ...
    #connections from Glutamatergic neurons of network 2 to TRN
    ...
    #connections from Glutamatergic neurons of network 1 to TRN
    ...
    for neuron_i in range(len(Glutneurons)):
        Glutneurons[neuron_i].soma.push()
        for neuron_j in range(len(GABAneurons_trn)):
            GlutGABAtneurons_sin1.append(h.NetCon(Glutneurons[neuron_i].soma(0.5)._ref_v,GABAneurons_trn[neuron_j].synE,0,5,GlutGABAtneurons_Wnet1trn[neuron_i,neuron_j]))
        h.pop_section()        
    
    #connectinos from GABAergic nuerons of TRN to network 1
    ...
    stim2 = h.NetCon(netStim[0],Glutneurons[1].synE,0.5,0,4./(100000))
    stim2 = h.NetCon(netStim[0],Glutneurons[2].synE,0.5,0,4./(100000))
    stim2 = h.NetCon(netStim[0],Glutneurons[3].synE,0.5,0,4./(100000))
    stim2 = h.NetCon(netStim[0],Glutneurons[4].synE,0.5,0,4./(100000))
    ...
    ofname = "./data_files/" "sim" + str(nsim+36) + ".txt"
    
    indx = np.arange(1,20001,40)
    
    x = np.array(tmpmean_TRN)
    y = np.array(tmpmean_V1)
    z = np.array(tmpmean_LGN)
    w = np.array(timeaxis)
    np.savetxt(ofname, (x[indx], y[indx], z[indx], w[indx]))
