...
with_TRN = False   #this includes collaterals to and from cortex
with_onlyLGNTRN = True   #this is to include only the LGN and the TRN
...
for nsim in range(nruns):
    ...
    for Gneuron_i in range(len(GABAneurons)):
        for neuron_i in range(len(Glutneurons)):
            ...
            GABAGlut_sin.append(h.NetCon(GABAneurons[Gneuron_i].soma(0.5)._ref_v,Glutneurons[neuron_i].synI,0,1,8./100000))
            h.pop_section()

            Glutneurons[neuron_i].soma.push()
            delayGlutGABA = 1
            #GlutGABA_sin.append(h.NetCon(Glutneurons[neuron_i].soma(0.5)._ref_v,GABAneurons[Gneuron_i].synE,0,delayGlutGABA,6./10000))   #this is to turn on the connection with the same synaptic weight as I to E
    ...
    if with_V1_L4:
        ...
        if connect_E_LGN_E_L4:
            ...
            GlutGlutneurons_W12 = GlutGlutneurons_W12 - np.diag(np.diag(GlutGlutneurons_W12))  # BUG: the diagonal should NOT be subtracted because this is a connections between two different network, not a net with itself
    ...
    if with_onlyLGNTRN:
        #create TRN neurons (inhibitory only) but only their connections to the LGN, not cortex
        NGABA_trn = 10
        GABAneurons_trn, GABAneurons_trn_rec = createnetwork(NGABA_trn)
        GABAneurons_trnW = np.random.exponential(1,NGABA_trn*NGABA_trn)*4/100000.
        GABAneurons_trnW = GABAneurons_trnW.reshape((NGABA_trn,NGABA_trn))
        ...
        #connections from Glutamatergic neurons of network 1 (LGN) to TRN
        GlutGABAtneurons_Wnet1trn = np.random.exponential(1,Nneurons*NGABA_trn)*8/1000000.
        GlutGABAtneurons_Wnet1trn = GlutGABAtneurons_Wnet1trn.reshape((Nneurons,NGABA_trn))
    ...
    #stim = h.NetCon(netStim[0],Glutneurons[0].synE,0.5,0,4./(100000))
    stim = h.NetCon(netStim[0],Glutneurons[0].synE,0.5,0,15./(100000))
    stim_rec = h.Vector()
    stim.record(stim_rec)
    #stim2 = h.NetCon(netStim[0],Glutneurons[1].synE,0.5,0,4./(100000))
    stim2 = h.NetCon(netStim[0],Glutneurons[1].synE,0.5,0,15./(100000))
    stim3 = h.NetCon(netStim[0],Glutneurons[2].synE,0.5,0,15./(100000))
    ...
    stim3 = h.NetCon(netStim[0],GABAneurons[0].synE,0.5,0,15./(100000))
    stim4 = h.NetCon(netStim[0],GABAneurons[1].synE,0.5,0,15./(100000))
    ...
    if with_TRN or with_onlyLGNTRN:
        ####################################
        #TRN LFP
